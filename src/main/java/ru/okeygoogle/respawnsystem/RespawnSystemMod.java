...
public final class RespawnSystemMod implements ModInitializer {
    ...
    private static final Set<String> THEMES = Set.of("SYSTEM", "CRIMSON", "VIOLET", "MONO");
    ...
    private static void handle(ServerPlayer player, String action, DataInputStream in) throws Exception {
        switch (action) {
            ...
            case "COSMETICS" -> cosmetics(player, ServerWire.clean(in.readUTF(), 16), ServerWire.clean(in.readUTF(), 9), ServerWire.clean(in.readUTF(), 60));
            ...
            case "MARKER_CREATE" -> {
                ...
                in.readUTF();
                in.readInt(); in.readInt(); in.readInt(); // координатам клиента не доверяем
            ...
            case "SYSTEM_ANNOUNCE" -> announcement(player, ServerWire.clean(in.readUTF(), 280));
    ...
    private static void initialNick(ServerPlayer player, String nick) {
        ...
        persistAndSync(player.getServer());
    }

    private static void cosmetics(ServerPlayer player, String theme, String accent, String subtitle) {
    ...
    private static void markerCreate(ServerPlayer player, String title, String type) {
        ...
        STORE.addMarker(player.getUUID(), p.systemName, title.isBlank() ? "Метка" : title,
                type.isBlank() ? "ОБЩАЯ" : type, dimension, pos.getX(), pos.getY(), pos.getZ());
        persistAndSync(player.getServer());
    }
    ...
    private static void ticketMessage(ServerPlayer player, int id, String message, boolean internal) {
        ...
        if ("ЗАКРЫТО".equals(ticket.status) && !role.canSupport()) {
            ServerWire.sendNotice(player, "Обращение уже закрыто.");
            return;
        }
        ProfileData p = STORE.getOrCreate(player.getUUID(), player.getName().getString());
        String sender = p.registered ? p.systemName : player.getName().getString();
        ticket.messages.add(new TicketMessage(System.currentTimeMillis(), sender, role.name(), message, internal));
        persistAndSync(player.getServer());
    }
    ...
    private static void announcement(ServerPlayer player, String text) {
        if (!role(player).canAdmin()) {
            ServerWire.sendNotice(player, "Недостаточный уровень доступа.");
            return;
        }
        if (text.isBlank()) return;
        STORE.addChat(new ChatEntry(System.currentTimeMillis(), "СИСТЕМА", text, true));
        persistAndSync(player.getServer());
    }
    ...
    private static void adminProfileUpdate(ServerPlayer actor, DataInputStream in) throws Exception {
        ...
        if (requestedRole == Role.OWNER && !isOpLike(actor)) {
            ServerWire.sendNotice(actor, "Только OP может назначить роль OWNER.");
        } else if (actorRole == Role.ADMIN && requestedRole.rank() >= Role.ADMIN.rank()) {
            ServerWire.sendNotice(actor, "Администратор может назначать роли только до MODERATOR.");
        } else {
        ...
        persistAndSync(actor.getServer());
    }
